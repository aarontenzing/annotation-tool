...
class ObjectPoseDataset(data.Dataset):
    ...
    def __init__(self, opt, split):
        ...
        # use random background as training
        if not opt.green_background:
            
            # specify the proper path and json data for the ford data: 
            if split == "train":
                self.img_dir="../data/random_background/train/"
                self.data_dir="../data/random_background/train"
                with open("../data/random_background/train/train_backgrounds.json") as f:
                        self.json_data= json.load(f)
            
            if split == "val":
                self.img_dir="../data/random_background/val/"
                self.data_dir="../data/random_background/val"
                with open("../data/random_background/val/val_backgrounds.json") as f:
                        self.json_data= json.load(f)
                        
        # use green background other jsons and other data 
        else:
            self.img_dir="../data/green_boxes/"
            if split == "train":
                self.data_dir="../data/green_boxes"
        ...
        # found using debug_pnp:
        self.bad_pointorder=['IMG_1040.JPG', 'IMG_1126.JPG','IMG_0674.JPG', 'IMG_0888.JPG', 'IMG_0847.JPG', 'IMG_0958.JPG', 'IMG_0598.JPG', 'IMG_0599.JPG', 'IMG_0616.JPG', 'IMG_0627.JPG', 'IMG_0646.JPG', 'IMG_0647.JPG', 'IMG_0648.JPG', 'IMG_0650.JPG', 'IMG_0652.JPG', 'IMG_0653.JPG', 'IMG_0658.JPG', 'IMG_0659.JPG', 'IMG_0660.JPG', 'IMG_0757.JPG', 'IMG_0796.JPG', 'IMG_0814.JPG', 'IMG_0778.JPG', 'IMG_0797.JPG', 'IMG_0795.JPG']
     
            
        self.img_objectron_path= "../data/outf_all/cereal"
        self.max_objs = 1 # only one object in the dataset
        ...
        def in_json(imgpath):            
            ...
            if name in self.bad_pointorder:
                return False
        ...
        #   Copy from DOPE code
        def loadimages(root, datastyle="json", extensions=['png', "JPG"], objectron=False):
            imgs = []
                    ...
                    for imgpath in glob.glob(path + "/*.{}".format(ext.replace('.', ''))):
                        ...
                        if not objectron:
                            ...
                            if exists(imgpath) and in_json(imgpath):
                                ...
                                imgs.append((imgpath, video_id, frame_id,"../data/example_json.json"))
                        else:
                            if exists(imgpath):
                                video_id = os.path.split(os.path.split(imgpath)[0])[1]
                                frame_id = os.path.splitext(os.path.basename(imgpath))[0]
        ...
        def load_data(path, extensions, objectron=False):
            
            imgs = loadimages(path, extensions=extensions, objectron=objectron)
            return imgs
        
        
        # load the data based on paths and json files 
        self.images = []
        self.img_objectron = []
        self.images += load_data(self.img_dir, extensions=["png", 'JPG'], objectron=False)
        self.num_samples = len(self.images)
        print("total num of samples: ", self.num_samples)
    ...
    def _get_aug_param(self, c_ori, s, width, height, disturb=False):
        """Get the image augmentation parrameters. 
        Rotation, scale, random crop
        

        Args:
            c_ori (float): the center of the image in floats
            s (float): the  bigges image dimention max( width, height) in float
            width (int): image width
            height (int): image height
            disturb (bool, optional): Defaults to False.
    ...
    """find box dimentions in csv file based on imgname"""
    def get_box_dim(name):
        dim=[0,0,0]
        print(name) 
        return dim
            
    def __getitem__(self, index):
        """Get a image and a annotation for training/testing.
        

        Args:
            index (int): the number of the img that needs to be fetecht. 
        """
        ...
        # with open("../data/outf_all/train/objectron/00002.json") as f:
        #     anns = json.load(f)
        num_objs=1
       # anns contains a lot of data see : print(anns)
        ...
        # only do this once because only one object per frame: 
        for k in range(num_objs):
            ...
            if "IMG" in name:  # working with Ford Data: 
                for dict in self.json_data:
                 ...
                 # invert y as if we want to traing green background
                if self.opt.green_background:
                ...
                # print("take points form objectron")
