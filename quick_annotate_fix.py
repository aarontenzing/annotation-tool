...
class quick_annotate:
    ...
    def calculate_cuboid(self, size, image_points, camera_matrix, dist_coeffs):
        # Calculate the 3D points of the object using PNP
        status = False
        ...
        while True:
            ...
            if key == ord('a'):
                ...
                if len(self.vertices) > 3 and len(self.vertices) <= 8:                       
                    ...
                    PNP_imagepoints, object_points, rotation_vector, translation_vector, dimensions, status = self.calculate_cuboid(self.boxSize, self.vertices, camera, dist)
                    ...
                    if status == True:
                        ...
                        if PNP_imagepoints is not None:
                            ...
                            self.data = {"img_name" : self.images[self.idx].split("\\")[-1], "projection": PNP_imagepoints.tolist(), "world": object_points.tolist(), "orientation": euler_angles.tolist(), "translation": translation_vector.flatten().tolist(), "dimensions": dimensions}
                        else:
                    ...
                    self.reset_image(self.images[self.idx])
                    # projected_points, point_3d_cam, scale, points_ori, bbox, stats = [], [], [], [], [], False
